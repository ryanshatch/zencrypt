...
# Get allowed directories from environment variables and resolve relative paths
ALLOWED_DIRS = [resolve_path(d.strip()) for d in os.getenv('ALLOWED_DIRECTORIES', './files').split(',')]
DEFAULT_FILE_PATH = resolve_path(os.getenv('DEFAULT_FILE_PATH', './files'))

# Create necessary directories if they don't exist
for directory in ALLOWED_DIRS:
    os.makedirs(directory, exist_ok=True)

def validate_path(filepath, allow_create=False):
    ...
    try:
        ...
        if not any(norm_path.startswith(allowed_dir) for allowed_dir in ALLOWED_DIRS):
            raise ValueError(f"Access denied: file path must be within allowed directories: {', '.join(ALLOWED_DIRS)}")
        
        # Create parent directories if allowed and they don't exist
        if allow_create:
            os.makedirs(os.path.dirname(norm_path), exist_ok=True)
            
        # For existing files, check if path exists and is a file
        elif not allow_create and os.path.exists(norm_path):
...
def safe_open_file(filepath, mode='r', allow_create=False):
    """Safely open files with path validation"""
    validated_path = validate_path(filepath, allow_create=(mode.startswith('w') or allow_create))
    # Make sure that the file is within the base directory
    abs_path = os.path.abspath(validated_path)
    if not abs_path.startswith(BASE_DIR):
        raise ValueError("Access denied: file path outside of allowed directory")
    return open(validated_path, mode, encoding='utf-8' if 'b' not in mode else None)

#* The name of the .key file.
KEY_FILE = os.path.join(BASE_DIR, "zen.key")                       # This is a private key file that is used to encrypt and decrypt the text.

#* Saves the key to a new key file with the default name: `zen.key`
def save_key_to_file(key):                 # saves the key to the key file
    with safe_open_file(KEY_FILE, "wb", allow_create=True) as key_file: # opens the key file in `write binary` mode
...
def load_key_from_file():                  # loads the key from the key file
    with safe_open_file(KEY_FILE, "rb") as key_file: # opens the key file in `read binary` mode
...
def encrypt_file(input_file, output_file, password):
    ...
    try:
        ...
        output_path = validate_path(output_file, allow_create=True)
        
        salt = os.urandom(16)
        key = generate_key(password, salt)
        iv = os.urandom(16)

        with safe_open_file(input_path, 'rb') as file:
            plaintext = file.read()
        ...
        with safe_open_file(output_path, 'wb', allow_create=True) as file:
...
def decrypt_file(input_file, output_file, password): 
    """Decrypts a file using AES symmetric encryption"""
    try:
        # Validate input and output paths
        input_path = validate_path(input_file)
        output_path = validate_path(output_file, allow_create=True)
...
def export_public_key_to_file(public_key, filename):
    """Exports the public key to a file in PEM format"""
    try:
        validated_path = validate_path(filename, allow_create=True)
        public_key_pem = public_key.public_bytes(
            encoding=serialization.Encoding.PEM,
            format=serialization.PublicFormat.SubjectPublicKeyInfo
        )

        with safe_open_file(validated_path, 'wb', allow_create=True) as f:
            f.write(public_key_pem)
    except ValueError as e:
        raise ValueError(f"Error exporting public key: {str(e)}")
...
def import_public_key_from_file(filename): # imports the public key from a file in PEM format
    """Imports the public key from a file in PEM format"""
    try:
        # Ensure the file is within allowed directories and exists
        ...
        with safe_open_file(validated_path, 'rb') as f:
            public_key_pem = f.read()
            
        # Validate the content is actually a public key
        try:
            return load_pem_public_key(public_key_pem, backend=default_backend())
        except ValueError:
            raise ValueError("Invalid public key format")
    except ValueError as e:
        raise ValueError(f"Error importing public key: {str(e)}")
...
def print_menu(sha256_hash):                        # This is the Main Menu for the Zencrypt CLI
    ...
    if answer == "1":                   # if the user selects option 1
        ...
        print("\n\nExiting Zencrypt...") # prints a message to the user that the program is exiting and closes after five seconds.
        time.sleep(5)                    # waits for five seconds
        os._exit(0)                      # exits the program after five seconds because 0 means the program has exited successfully, but it never waited five seconds..
    else:                               # if the user enters an invalid input for error handling
        print("\nInvalid Input.")       # prints a message to the user that the input is invalid
